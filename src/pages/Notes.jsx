...
export default function Notes() {
  ...
  // ownerEmail stays from config — used for auth logic only, not data display
  ...
  const [form, setForm]                       = useState({ animal: '', note_type: 'observation', content: '' })
  ...
  // ── Load notes and animal names in parallel ──────────────────
  async function loadAll() {
    setLoading(true)
    try {
      const [noteDocs, animalDocs] = await Promise.all([
        listAll(COL.ranchNotes, [Query.orderDesc('$createdAt')]),
        getAnimals(),
      ])
      setNotes(noteDocs)

      // Animal names from Appwrite — excludes archived animals
      const names = (animalDocs || [])
  ...
  async function save() {
    ...
    try {
      await createDoc(COL.ranchNotes, {
        author:     user?.name || 'Sitter',
        note_type:  form.note_type,
        animal:     form.animal || '',
        content:    form.content,
        photo_urls: uploadedUrls,
      })
      // Refetch so every device stays in sync
      await loadAll()
    } catch {
      ...
      setSaving(false)
      setShowModal(false)
      setForm({ animal: '', note_type: 'observation', content: '' })
      setPhotoFiles([])
      setPhotoPreviews([])
    }
  }
      ...
      {notes.map(note => (
        <div
          ...
          style={{ borderLeft: `4px solid ${
            note.note_type === 'concern'  ? 'var(--danger)'
            : note.note_type === 'supply' ? 'var(--forest-green)'
            : 'var(--sky-blue)'
          }` }}
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span className={`badge ${NOTE_TYPES[note.note_type]?.badge}`}>
