// ============================================================
// 🐴 RANCH CONFIGURATION — REBALOVE RANCH
// ============================================================
//
// ── WHAT THIS FILE IS ──────────────────────────────────────
// This file has TWO jobs only:
//
//   1. SEED DATA — arrays used by ranchDataService.js to
//      populate Appwrite collections on first deployment.
//      After seeding, the UI reads EXCLUSIVELY from Appwrite.
//      Editing these arrays after first deploy has NO effect
//      on the running app — edit data in the Admin Panel.
//
//   2. RUNTIME CONSTANTS — a small set of values read
//      directly by components at runtime (ownerEmail,
//      vehicleNotes, evacuationSteps, manure display text).
//      These are noted clearly below.
//
// ── HOW TO DEPLOY A NEW RANCH ──────────────────────────────
//   1. Duplicate this project
//   2. Replace ALL values below with the new ranch's info
//   3. Create a new Appwrite project + collections
//   4. Deploy to a new Vercel project
//   5. Load data via Admin Panel → "Load from Config" buttons
//   Done! New ranch live in ~30 minutes.
//
// ── RULE: Never import seed arrays in UI components ────────
//   ✅ OK:  import { RANCH_CONFIG } then use ownerEmail,
//           vehicleNotes, evacuationSteps, manure
//   ❌ NOT OK: importing animals, feedSchedule, dailyTasks,
//           propertyTasks, treats, waterNotes, emergencyContacts
//           directly in any page component
// ============================================================

export const RANCH_CONFIG = {

  // ── BRANDING ──────────────────────────────────────────────
  // RUNTIME: Used in Layout.jsx and LoginScreen.jsx
  name:       'RebaLove Ranch',
  shortName:  'RLR',
  tagline:    'Ranch Sitter App',
  location:   'Sierra Nevada Foothills, CA',
  ownerName:  'Karen',
  ownerEmail: 'kmatthews4240@gmail.com', // ← Determines who gets Admin tab

  // ── RUNTIME CONSTANTS ─────────────────────────────────────
  // These are read directly by components at runtime.
  // They have no Appwrite collection — edit here to change them.

  // Used by Emergency.jsx and Evacuation.jsx
  vehicleNotes: {
    keysLocation:  'In the dish on the coat rack by the back door',
    trailerStatus: 'Trailer is already hitched and ready to go',
  },

  // Used by Evacuation.jsx — the 7 step-by-step instructions
  evacuationSteps: [
  ...
  // Used by ManureLog.jsx — display text only, not stored in Appwrite
  manure: {
    frequencyDays:    3,
    frequencyLabel:   '3–4',
  ...
  // ── SEED DATA ─────────────────────────────────────────────
  // Used ONCE by ranchDataService.js to populate Appwrite.
  // After first deploy, edit data via the Admin Panel instead.
  // ⚠️  Do NOT import these arrays in UI components directly.

  animals: [
    {
      name: 'Shadow', type: 'equine', breed: 'Horse', emoji: '🐴',
    ...
    {
      name: 'Luke', type: 'equine', breed: 'Donkey', emoji: '🫏',
    ...
    {
      name: 'Snowy', type: 'equine', breed: 'Horse', emoji: '🐴',
      notes: 'Calm and cooperative.',
      likes: 'Carrots (cut in 1" slices), apples (cut in 1x1" pieces)',
      dislikes: null, odd_but_ok: null,
      evacuationOrder: 2,
    ...
    {
      name: 'Shiloh', type: 'dog', breed: 'Dog', emoji: '🐕',
      notes: 'Sweet girl.',
      ...
      special: ['⚠️ NO water after 6:00 PM or she wets the bed.'],
    },
    {
      name: 'Chickens', type: 'chicken', breed: 'Mixed flock', emoji: '🐔',
    ...
    {
      name: 'Cats', type: 'cat', breed: 'Ranch cats', emoji: '🐈',
      notes: 'Outdoor / Mud Room cats.',
      likes: null, dislikes: 'Being inside the house', odd_but_ok: null,
      evacuationOrder: null,
      special: [
  ...
  feedSchedule: [
    {
      animal: 'Luke (🫏 Donkey)',
      meals: [
        { time: 'breakfast',   window: '6:30–7:30 AM', items: '1 bag breakfast pellets + 1 flake hay. BEFORE feeding: give ½ Pergolide (square pink pill) + 3 Thyro-L capsules hidden in a handful of chicken scratch — give by hand.', hasMed: true },
        { time: 'dinner',      window: '4:00–5:30 PM', items: '1 bag pellets + 1 flake hay. BEFORE dinner: give 3 Thyro-L capsules hidden in a handful of chicken scratch — give by hand.', hasMed: true },
        { time: 'night_check', window: '8:00 PM',      items: 'Check hay nets (fill if less than half full). Give 3 Thyro-L capsules hidden in a handful of chicken scratch — give by hand.', hasMed: true },
      ],
    },
    {
      animal: 'Luke — Noon Medication Only',
      meals: [
        { time: 'dinner', window: '~Noon', items: '3 Thyro-L capsules hidden in a handful of chicken scratch — give by hand. Mid-day medication only, not a full feeding.', hasMed: true },
      ],
    },
    {
      animal: 'Snowy',
    ...
    },
    {
      animal: 'Chickens',
      meals: [
        { time: 'breakfast',   window: '6:30–7:30 AM', items: 'Hang gravity feeder back out (was stored inside overnight). Check feeder & water. Give 6 scoops scratch + 3 scoops mealworms — this is required, not optional.' },
        { time: 'dinner',      window: '4:00–5:30 PM', items: 'Check feeder & water. Give 6 scoops scratch + 3 scoops mealworms.' },
        { time: 'night_check', window: '~8:30 PM',     items: 'Confirm all hens in coop, close door if open (auto-closes 9pm). 🗄️ Bring gravity feeder INSIDE the cabinet for the night — prevents skunks!' },
      ],
    },
    {
      animal: 'Dogs',
  ...
  propertyTasks: [
    ...
    { id: '2', title: 'Spread Manure', description: 'Go SLOWLY — lots of gravel mixed in. Listen for rock jams (wheels stop turning). Rock jam fix: reverse a few inches. The clutch bolt will break if you push through.', category: 'every_few_days', frequency_days: 3, priority: 'normal', supply_location: 'Manure spreader in barn', warning: 'Rock jam hazard! Listen carefully. Reverse to clear. Never force through.' },
    ...
    { id: '4', title: 'Night Check — Lock Chicken Coop', description: 'Around 8:30pm — confirm all hens inside, close door if open. Auto-close is 9pm. Bring gravity feeder inside cabinet.', category: 'daily', frequency_days: 1, priority: 'high' },
  ...
  treats: [
    { animals: 'Luke (🫏), Snowy & Shadow', emoji: '🐴', description: '3–4 medium carrots cut into 1" slices + 1 apple cut into 1x1" pieces — shared among all three equines' },
    { animals: 'Chickens', emoji: '🐔', description: 'A handful of grapes OR blueberries. No additional scratch or mealworms with the fruit treat.' },
  ],
  ...
  emergencyContacts: [
    {
      category: 'vets', title: '🏥 Veterinary',
      items: [
...
// ── END OF CONFIG ─────────────────────────────────────────────
// No derived exports below — EVACUATION_ANIMALS, ANIMAL_NAMES,
// and animalsByType have been removed. These are now derived
// at runtime from Appwrite queries in the relevant components.
